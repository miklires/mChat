...
public class LocalCommand implements CommandExecutor {
    ...
    public LocalCommand(MChat plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (!player.hasPermission("mchat.command.local")) {
            player.sendMessage(MiniMessage.miniMessage().deserialize("<red>Нет прав."));
            return true;
        }
        if (args.length < 1) {
            player.sendMessage(MiniMessage.miniMessage().deserialize("<red>Использование: /l <сообщение>"));
            return true;
        }
        ...
        plugin.getMessageRouter().sendChat(player, sb.toString(), false);
        return true;
    }
}
